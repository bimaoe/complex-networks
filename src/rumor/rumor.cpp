...
long long Rumor::Evolution::getSpreaderCount(void) {
...
Rumor::Evolution * Rumor::runISR(Graph & graph, double delta, double nu,
    vector<int> & initiallySpreader, int maxIterations) {
  list<int> spreaderVertices;
  vector<Status> status(graph.getVertexCount(), IGNORANT);
    ...
    for (auto it = spreaderVertices.begin(); it != spreaderVertices.end(); it++) {
      ...
      for (auto it2 = shuffledNeighbours.begin(); it2 != shuffledNeighbours.end(); it2++) {
        ...
        if (status[neighbour] == IGNORANT) {
          if (Stat::uniform() < delta) {
            // Spreading.
            status[neighbour] = SPREADER;
            spreaderVertices.push_front(neighbour);
            iCount--;
            sCount++;
          }
        } else {
...
Rumor::Evolution * Rumor::runISR(Graph & graph, double delta, double nu, int firstInfected,
    int maxIterations) {
  vector<int> initiallySpreader;
  initiallySpreader.push_back(firstInfected);
  return runISR(graph, delta, nu, initiallySpreader, maxIterations);
}
